...
@implementation ZRAssetExportSession

+ (instancetype)exportSessionWithAsset:(AVAsset *)asset
{
    return [[ZRAssetExportSession alloc] initWithAsset:asset];
}
...
- (void)setOutputFileType:(NSString *)outputFileType {
    ...
    if (_outputFileType == AVFileTypeMPEG4) {    
        int videoWidth = 540;
        int videoHeight = 960;
        float bitsPerPixel = 2.5; //每像素比特率       建议4-6,超高清
        float bitsPerSecond = videoWidth * videoHeight; //每秒比特率
        ...
        AVVideoCompressionPropertiesKey: @
            {
            AVVideoAverageBitRateKey: @(bitsPerSecond * bitsPerPixel),
            AVVideoProfileLevelKey: AVVideoProfileLevelH264Main41, //AVVideoProfileLevelH264MainAutoLevel,
            },
        };
...
+ (NSString *)generateAVAssetTmpPath {
    NSString * myPathDocs =  [NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"ZR-Lower-Bit-Rate-%d.mp4",arc4random() % 100000]];
